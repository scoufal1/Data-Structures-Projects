 ...
 * Desc:
 *
 * Implementation of a hash table that uses a double hash function, based on the book's
 * implementation of linear hash table. Also contains variables to count the average 
 * number of probes and max number of probes.
 * 
 */
...
public class DoubleHashMap<K,V> extends AbstractHashMap<K,V> {
    ...
    //prime used for double hashing
    public static final int PRIME = 500009;
    ...
    private double probes; //counts how many probes occur during findslot
    private int maxProbes; //counts the maximum amount of probes in one call of findslot

    //resets instance variables to zero
    ...
    /** Calculates the average number of probes during some action (insertion,
     * search, etc)
     * @param size The size of the action (number of insertions attempted if insertion)
     * @return Division of total probes by total size
     */
    public double averageProbes(int size) {
	return probes/size;
    }
	...
	int max = 0; //initialize to 0 when method is called
	do {   
	    probes++;
	    max++;
		...
		//set maxProbes to max if it is greater than current maxProbes
		if(max > maxProbes) {
		    maxProbes = max;
		}
		return j;
	    } // successful match                     
	    int doubleHash = doubleHash(h);
	    j = (j + doubleHash) % capacity;          // keep looking (cyclically)
    ...
    /** Rehashes the key to prevent clustering
     * @param k The key
     * @return the second hash value (size of jump)
     */
    private int doubleHash(int h) {
	int doubleHash = PRIME - (h % PRIME);
	return doubleHash;	
    }
